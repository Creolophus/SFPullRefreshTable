...
@property CGFloat loadingOffset;

@property (assign, nonatomic) CGFloat orignContentOffsetY; //if navigationBar is translucent, table will have orign contentOffset and orign contentInset
...
- (void)tableViewDidScroll{
    ...
    if (_prType == PRTypeTopLoad) {
        ...
        if (offset.y<0) {
            ...
            _loadingOffset = self.contentOffset.y;
            if (_loadHandler) {
                _loadHandler();
            }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([keyPath isEqualToString:KeyPathContentSize]) {
        ...
        if (_prType == PRTypeTopLoad) {
            ...
            if (curContentHeight-preContentHeight>0 && preContentHeight>0 && curContentHeight>0) {
                CGPoint offset = self.contentOffset;
    ...
    {
        if (_orignContentOffsetY <-1000) { // required, before table adding to the window, if table offset changed, should return
            return;
        }
